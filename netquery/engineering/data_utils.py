...
def load_graph(data_dir, embed_dim):
    ...
    #The loaded data must be a bytes-like object.
    rels, adj_lists, node_maps = pickle.load(open(data_dir+"/graph_data.pkl", "rb"))
    #rels={'function':[(, ),...],'drug':[(,),...],'protein':[(,),...],'disease':[(,),...],'sideeffects':[(,),...]}
    #node_maps={'function':[function_num],'drug':[drug_num],'protein':[protein_num],'disease':[disease_num],'sideeffects':[sideeffects_num]}
    node_maps = {m : {n : i for i, n in enumerate(id_list)} for m, id_list in node_maps.items()}
    #node_maps={'function':{'function_num':function_idx,...},'drug':{'drug_num':drug_idx,...},'disease':{'disease_num':disease_idx,...},'sideeffects':{'sideeffects_num':sideeffects_idx,...}}
    for m in node_maps:
        node_maps[m][-1] = -1
